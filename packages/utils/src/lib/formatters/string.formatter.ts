import { ArrayHelper } from '../helpers/array.helper';
import { NumberFormatter } from './number.formatter';
import { ObjectHelper } from '../helpers/object.helper';

export type NullableString = string | undefined | null;

export class StringFormatter {
  public static readonly EMPTY: string = '';
  public static readonly INDEX_NOT_FOUND: number = -1;

  /**
   * check current string is empty.
   * @param str
   * @example StringFormatter.isEmpty(null)      = true
   * @example StringFormatter.isEmpty(undefined) = true
   * @example StringFormatter.isEmpty("")        = true
   * @example StringFormatter.isEmpty(" ")       = false
   * @example StringFormatter.isEmpty("bob")     = false
   * @example StringFormatter.isEmpty("  bob  ") = false
   */
  public static isEmpty(str: NullableString): boolean {
    return ObjectHelper.isNullOrUndefined(str) || str.length === 0;
  }

  /**
   * check current string is not empty.
   * @param str
   * @example StringFormatter.isNotEmpty(null)      = false
   * @example StringFormatter.isNotEmpty(undefined) = false
   * @example StringFormatter.isNotEmpty("")        = false
   * @example StringFormatter.isNotEmpty(" ")       = true
   * @example StringFormatter.isNotEmpty("bob")     = true
   * @example StringFormatter.isNotEmpty("  bob  ") = true
   */
  public static isNotEmpty(str: NullableString): str is NonNullable<NullableString> {
    return !this.isEmpty(str);
  }

  /**
   * check current string is blank.
   * @param str
   * @example StringFormatter.isBlank(null)      = true
   * @example StringFormatter.isBlank(undefined) = true
   * @example StringFormatter.isBlank("")        = true
   * @example StringFormatter.isBlank(" ")       = true
   * @example StringFormatter.isBlank("bob")     = false
   * @example StringFormatter.isBlank("  bob  ") = false
   */
  public static isBlank(str: NullableString): boolean {
    return ObjectHelper.isNullOrUndefined(str) || str.trim() === this.EMPTY;
  }

  /**
   * check current string is not blank.
   * @param str
   * @example StringFormatter.isNotBlank(null)      = false
   * @example StringFormatter.isNotBlank(undefined) = false
   * @example StringFormatter.isNotBlank("")        = false
   * @example StringFormatter.isNotBlank(" ")       = false
   * @example StringFormatter.isNotBlank("bob")     = true
   * @example StringFormatter.isNotBlank("  bob  ") = true
   */
  public static isNotBlank(str: NullableString): str is NonNullable<NullableString> {
    return !this.isBlank(str);
  }

  /**
   * Removes all leading and trailing white-space characters from the current string.
   * @param str
   * @example StringFormatter.trim(null)        = null
   * @example StringFormatter.trim(undefined)   = undefined
   * @example StringFormatter.trim("")          = ""
   * @example StringFormatter.trim("    ")      = ""
   * @example StringFormatter.trim(" a   ")     = "a"
   */
  public static trim(str: string): string {
    if (ObjectHelper.isNullOrUndefined(str)) {
      return str;
    } else {
      return str.replace('\b', this.EMPTY).trim();
    }
  }

  /**
   * Removes all leading and trailing white-space characters from the current string to null.
   * @param str
   * @example StringFormatter.trimToNull(null)             = null
   * @example StringFormatter.trimToNull(undefined)        = null
   * @example StringFormatter.trimToNull("")               = null
   * @example StringFormatter.trimToNull("    ")           = null
   * @example StringFormatter.trimToNull(" a   ")          = "a"
   */
  public static trimToNull(str: string): string | null {
    if (ObjectHelper.isNullOrUndefined(str)) {
      return null;
    }
    const tmp = this.trim(str);
    return this.isBlank(tmp) ? null : tmp;
  }

  /**
   * Removes all leading and trailing white-space characters from the current string to "".
   * @param str
   * @example StringFormatter.trimToNull(null)             = ""
   * @example StringFormatter.trimToNull(undefined)        = ""
   * @example StringFormatter.trimToNull("")               = ""
   * @example StringFormatter.trimToNull("    ")           = ""
   * @example StringFormatter.trimToNull(" a   ")          = "a"
   */
  public static trimToEmpty(str: string): string {
    if (ObjectHelper.isNullOrUndefined(str)) {
      return this.EMPTY;
    }
    const tmp = this.trim(str);
    return this.isBlank(tmp) ? this.EMPTY : tmp;
  }

  /**
   * Strips any of a set of characters from the start and end of a String.
   * @param str
   * @param stripChars
   */
  public static strip(str: string, stripChars?: string): string {
    const tmp = this.stripStart(str, stripChars);
    return this.stripEnd(tmp, stripChars);
  }

  /**
   * Strips whitespace from the start and end of a String returning null if the String is empty ("") after the strip.
   * @param str
   */
  public static stripToNull(str: string): string | null {
    if (ObjectHelper.isNullOrUndefined(str)) {
      return null;
    }
    const tmp = this.strip(str);
    return this.isBlank(tmp) ? null : tmp;
  }

  /**
   * Strips whitespace from the start and end of a String returning an empty String if null input.
   * @param str
   */
  public static stripToEmpty(str: string): string {
    if (ObjectHelper.isNullOrUndefined(str)) {
      return this.EMPTY;
    }
    const tmp = this.strip(str);
    return this.isBlank(tmp) ? this.EMPTY : tmp;
  }

  /**
   * Strips any of a set of characters from the start of a String.
   * @param str
   * @param stripChars
   * @example StringFormatter.stripStart(null, *)          = null
   * @example StringFormatter.stripStart("", *)            = ""
   * @example StringFormatter.stripStart("abc", "")        = "abc"
   * @example StringFormatter.stripStart("abc", null)      = "abc"
   * @example StringFormatter.stripStart("  abc", null)    = "abc"
   * @example StringFormatter.stripStart("abc  ", null)    = "abc  "
   * @example StringFormatter.stripStart(" abc ", null)    = "abc "
   * @example StringFormatter.stripStart("yxabc  ", "xyz") = "abc  "
   */
  public static stripStart(str: string, stripChars?: string): string {
    if (ObjectHelper.isNullOrUndefined(str) || str.length === 0) {
      return str;
    }
    const strLen = str.length;
    let start = 0;
    if (ObjectHelper.isNullOrUndefined(stripChars)) {
      while (start !== strLen && this.isWhitespace(str.charAt(start))) {
        start++;
      }
    } else if (stripChars.length === 0) {
      return str;
    } else {
      while (start !== strLen && stripChars.indexOf(str.charAt(start)) !== -1) {
        start++;
      }
    }
    return str.substring(start);
  }

  /**
   * Strips any of a set of characters from the end of a String.
   * @param str
   * @param stripChars
   * @example StringFormatter.stripEnd(null, *)          = null
   * @example StringFormatter.stripEnd("", *)            = ""
   * @example StringFormatter.stripEnd("abc", "")        = "abc"
   * @example StringFormatter.stripEnd("abc", null)      = "abc"
   * @example StringFormatter.stripEnd("  abc", null)    = "  abc"
   * @example StringFormatter.stripEnd("abc  ", null)    = "abc"
   * @example StringFormatter.stripEnd(" abc ", null)    = " abc"
   * @example StringFormatter.stripEnd("  abcyx", "xyz") = "  abc"
   */
  public static stripEnd(str: string, stripChars?: string): string {
    if (ObjectHelper.isNullOrUndefined(str) || str.length === 0) {
      return str;
    }
    let end = str.length;
    if (stripChars == null) {
      while (end !== 0 && this.isWhitespace(str.charAt(end - 1))) {
        end--;
      }
    } else if (stripChars.length === 0) {
      return str;
    } else {
      while (end !== 0 && stripChars.indexOf(str.charAt(end - 1)) !== -1) {
        end--;
      }
    }
    return str.substring(0, end);
  }

  /**
   * Compares two CharSequences, returning true if they represent equal sequences of characters.
   * @param str1
   * @param str2
   * @example StringFormatter.equal(null, null)              = true
   * @example StringFormatter.equal(undefined, undefined)      = true
   * @example StringFormatter.equal(undefined, null)          = false
   * @example StringFormatter.equal(null, undefined)          = false
   * @example StringFormatter.equal(null, "abc")             = false
   * @example StringFormatter.equal("abc", null)             = false
   * @example StringFormatter.equal("abc", undefined)         = false
   * @example StringFormatter.equal(undefined, "abc")         = false
   * @example StringFormatter.equal("abc", "def")            = false
   * @example StringFormatter.equal("abc", "abc")            = true
   */
  public static equals(str1: string, str2: string): boolean {
    if (!ObjectHelper.isNullOrUndefined(str1) && !ObjectHelper.isNullOrUndefined(str2)) {
      return str1 === str2;
    }
    if (ObjectHelper.isNull(str1) && ObjectHelper.isNull(str2)) {
      return true;
    }
    if (ObjectHelper.isUndefined(str1) && ObjectHelper.isUndefined(str2)) {
      return true;
    }
    return false;
  }

  /**
   * Compares two CharSequences, returning true if they represent equal sequences of characters, ignoring case.
   * @param str1
   * @param str2
   * @example StringFormatter.equal(null, null)              = true
   * @example StringFormatter.equal(undefined, undefined)      = true
   * @example StringFormatter.equal(undefined, null)          = false
   * @example StringFormatter.equal(null, undefined)          = false
   * @example StringFormatter.equal(null, "abc")             = false
   * @example StringFormatter.equal("abc", null)             = false
   * @example StringFormatter.equal("abc", undefined)         = false
   * @example StringFormatter.equal(undefined, "abc")         = false
   * @example StringFormatter.equal("abc", "def")            = false
   * @example StringFormatter.equal("abc", "abc")            = true
   * @example StringFormatter.equal("abc", "AbC")            = true
   */
  public static equalsIgnoreCase(str1: string, str2: string): boolean {
    if (!ObjectHelper.isNullOrUndefined(str1) && !ObjectHelper.isNullOrUndefined(str2)) {
      return str1.toLocaleLowerCase() === str2.toLocaleLowerCase();
    }
    if (ObjectHelper.isNull(str1) && ObjectHelper.isNull(str2)) {
      return true;
    }
    if (ObjectHelper.isUndefined(str1) && ObjectHelper.isUndefined(str2)) {
      return true;
    }
    return false;
  }

  /**
   * Finds the first index within a CharSequence, handling null.
   * @param str
   * @param searchStr
   * @param startPos
   * @example StringFormatter.indexOf(null, *)         = -1
   * @example StringFormatter.indexOf(undefined, *)         = -1
   * @example StringFormatter.indexOf("", *)           = -1
   * @example StringFormatter.indexOf("aabaabaa", 'a') = 0
   * @example StringFormatter.indexOf("aabaabaa", 'b') = 2
   * @example StringFormatter.indexOf("aabaabaa", 'b', 3) = 5
   * @example StringFormatter.indexOf("aabaabaa", '') = 0
   */
  public static indexOf(str: string, searchStr: string, startPos?: number): number {
    if (
      ObjectHelper.isNullOrUndefined(str) ||
      ObjectHelper.isNullOrUndefined(searchStr) ||
      (!ObjectHelper.isUndefined(startPos) && !NumberFormatter.isSafeInteger(startPos))
    ) {
      return this.INDEX_NOT_FOUND;
    }
    const useStartPos = ObjectHelper.isUndefined(startPos) ? 0 : startPos;
    return str.indexOf(searchStr, useStartPos);
  }

  /**
   * Returns the index within seq of the first occurrence of the specified character, starting the search at the specified index.
   * @param str
   * @param searchStr
   * @param position
   * @example StringFormatter.lastIndexOf("aFkyk", "k")          =4
   * @example StringFormatter.lastIndexOf("a Fkyk", " ");        =1
   * @example StringFormatter.lastIndexOf("aabaabaa", "b");      =5
   * @example StringFormatter.lastIndexOf("aabaabaa", "b", 4);   =2
   */
  public static lastIndexOf(str: string, searchStr: string, position?: number): number {
    if (
      ObjectHelper.isNullOrUndefined(str) ||
      ObjectHelper.isNullOrUndefined(searchStr) ||
      (!ObjectHelper.isUndefined(position) && !NumberFormatter.isSafeInteger(position))
    ) {
      return this.INDEX_NOT_FOUND;
    }
    const usePosition = ObjectHelper.isUndefined(position) ? str.length - 1 : position;
    return str.lastIndexOf(searchStr, usePosition);
  }

  /**
   * Checks if CharSequence contains a search character, handling null.
   * @param str
   * @param searchStr
   */
  public static contains(str: string, searchStr: string): boolean {
    if (ObjectHelper.isString(str) && ObjectHelper.isString(searchStr)) {
      return str.indexOf(searchStr) >= 0;
    } else {
      return false;
    }
  }

  /**
   * Checks if CharSequence contains a search CharSequence irrespective of case, handling null.
   * @param str
   * @param searchStr
   */
  public static containsIgnoreCase(str: string, searchStr: string): boolean {
    if (ObjectHelper.isString(str) && ObjectHelper.isString(searchStr)) {
      return str.toLocaleLowerCase().indexOf(searchStr.toLocaleLowerCase()) >= 0;
    } else {
      return false;
    }
  }

  /**
   * Gets a substring from the specified String avoiding exceptions.
   * @param str
   * @param start
   * @param end
   */
  public static subString(str: string, start: number, end?: number): string | null {
    if (
      !ObjectHelper.isString(str) ||
      !NumberFormatter.isSafeInteger(start) ||
      (!ObjectHelper.isUndefined(end) && !NumberFormatter.isSafeInteger(end))
    ) {
      return null;
    }

    if (str.length === 0) {
      return str;
    }

    if (ObjectHelper.isUndefined(end)) {
      return str.substring(start);
    } else {
      return str.substring(start, end);
    }
  }

  /**
   * Determines whether the beginning of this string instance matches the specified string.
   * @param str
   * @param prefix
   */
  public static startWith(str: string, prefix: string): boolean {
    if (!ObjectHelper.isString(str) || !ObjectHelper.isString(prefix)) {
      return false;
    }
    // return str.indexOf(prefix) === 0;
    // according to https://leonax.net/p/5806/use-string-startswith-and-endswith-in-javascript/
    return str.slice(0, prefix.length) === prefix;
  }

  /**
   * Determines whether the beginning of this string instance matches the specified string irrespective of case.
   * @param str
   * @param prefix
   */
  public static startWithIgnoreCase(str: string, prefix: string): boolean {
    if (!ObjectHelper.isString(str) || !ObjectHelper.isString(prefix)) {
      return false;
    }
    const useStr = str.toLocaleLowerCase();
    const usePrefix = prefix.toLocaleLowerCase();
    return useStr.slice(0, usePrefix.length) === usePrefix;
  }

  /**
   *  Determines whether the end of this string instance matches the specified string.
   * @param str
   * @param suffix
   */
  public static endWith(str: string, suffix: string): boolean {
    if (!ObjectHelper.isString(str) || !ObjectHelper.isString(suffix)) {
      return false;
    }
    return str.indexOf(suffix, str.length - suffix.length) !== -1;
  }

  /**
   * Determines whether the end of this string instance matches the specified string irrespective of case.
   * @param str
   * @param suffix
   */
  public static endWithIgnoreCase(str: string, suffix: string): boolean {
    if (!ObjectHelper.isString(str) || !ObjectHelper.isString(suffix)) {
      return false;
    }
    const useStr = str.toLocaleLowerCase();
    const useSuffix = suffix.toLocaleLowerCase();
    return useStr.indexOf(useSuffix, useStr.length - useSuffix.length) !== -1;
  }

  /**
   * check if current char is whitespace
   * @param ch
   */
  public static isWhitespace(ch: string): boolean {
    return ' \f\n\r\t\v\u00A0\u2028\u2029'.indexOf(ch) > -1;
  }

  /**
   * Initializes a new string of the guid structure.
   */
  public static newGuid(): string {
    return (
      this.S4() +
      this.S4() +
      '-' +
      this.S4() +
      '-' +
      this.S4() +
      '-' +
      this.S4() +
      '-' +
      this.S4() +
      this.S4() +
      this.S4()
    );
  }

  private static S4() {
    return (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1);
  }

  /**
   * JavaScript convert string to snake_case.
   * @example console.log(toSnakeCase('string')); // => string
   * @example console.log(toSnakeCase('camelCase')); // => camel_case
   * @example console.log(toSnakeCase('param-case')); // => param_case
   * @example console.log(toSnakeCase('PascalCase')); // => pascal_case
   * @example console.log(toSnakeCase('UPPER_CASE')); // => upper_case
   * @example console.log(toSnakeCase('snake_case')); // => snake_case
   * @example console.log(toSnakeCase('sentence case')); // => sentence_case
   * @example console.log(toSnakeCase('Title Case')); // => title_case
   * @example console.log(toSnakeCase('dot.case')); // => // dot_case
   * @example
   * @example console.log(toSnakeCase('')); // => ''
   * @example console.log(toSnakeCase(null)); // => ''
   * @example console.log(toSnakeCase(undefined)); // => ''
   * @example
   * @example console.log(toSnakeCase('Abc ___ 123 ___ xYz')); // => abc_123_x_yz
   * @example console.log(toSnakeCase('123__abc  ... ?// {#} def 12')); // => 123_abc_def_12
   * @example console.log(toSnakeCase('	tab space ??? ________')); // => tab_space
   * @example console.log(toSnakeCase('___?||123  abc|| 123..123')); // => 123_abc_123_123
   * @example console.log(toSnakeCase('!@#$%  {}|":;" ABC XyZ G123H')); // => abc_xy_z_g123h
   * @example console.log(toSnakeCase(' ^&* #DEFine x: 15 + ==')); // => define_x_15
   * @example console.log(toSnakeCase('123456789')); // => 123456789
   */
  public static snakeCase(str: string): string {
    if (this.isBlank(str)) {
      return '';
    }

    return String(str)
      .replace(/^[^A-Za-z0-9]*|[^A-Za-z0-9]*$/g, '')
      .replace(/([a-z])([A-Z])/g, (m, a, b) => a + '_' + b.toLowerCase())
      .replace(/[^A-Za-z0-9]+|_+/g, '_')
      .toLowerCase();
  }

  /**
   * Replaces all text find in a string.
   * @example StringFormatter.replaceAll(null, "a", "b")    = null;
   * @example StringFormatter.replaceAll(" ", " ", "a")    = "a";
   * @example StringFormatter.replaceAll("aa", "a", "b")    = "bb";
   */
  public static replaceAll(str: string, searchValue: string, replacer: string): string {
    if (
      !ObjectHelper.isString(str) ||
      !ObjectHelper.isString(searchValue) ||
      !ObjectHelper.isString(replacer)
    ) {
      return str;
    }

    return str.replace(new RegExp(this.escapeRegExp(searchValue), 'g'), replacer);
  }

  /**
   * Joins the elements of the provided array into a single string.
   * @param array - The array of values to join together, may be null or undefined.
   * @param separator - The separator character to use, null treated as "".
   * @returns The joined string.
   * @example StringFormatter.join([], *)               = "";
   * @example StringFormatter.join(["a", "b", "c"], null) = "abc"
   * @example StringFormatter.join(["a", "b", "c"], "-")  = "a-b-c"
   * @example StringFormatter.join([1, 2, 3], "-")       = "1-2-3"
   */
  public static join<T>(array: T[], separator: string | null = ''): string {
    if (ArrayHelper.isEmpty(array)) {
      return '';
    }

    const useSeparator = separator ?? '';
    return array.map(item => String(item)).join(useSeparator);
  }

  private static escapeRegExp(str: string): string {
    return str.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, '\\$1');
  }
}
