...
import { DiscountCalculation } from '../../../models/pricing.types';
...
export class DiscountCalculator {
  ...
  calculateDiscountAmount(originalPrice: number, discountPercentage: number): number {
    try {
      ...
      const percentage = DecimalUtils.divideAsNumber(discountPercentage, 100);
      return DecimalUtils.multiplyAsNumber(originalPrice, percentage);
    } catch (error) {
  ...
  applyDiscount(originalPrice: number, discountPercentage: number): number {
    try {
      ...
      return DecimalUtils.subtractAsNumber(originalPrice, discountAmount);
    } catch (error) {
  ...
  calculateDiscountPercentage(originalPrice: number, discountAmount: number): number {
    try {
      ...
      const percentage = DecimalUtils.divideAsNumber(discountAmount, originalPrice);
      return DecimalUtils.multiplyAsNumber(percentage, 100);
    } catch (error) {
